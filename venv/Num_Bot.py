...
@dp.message(F.text.lower().in_(['да', 'давай', 'сыграем', 'играть', 'игра', 'хочу играть']))
...
async def process_numbers_answer(message: Message):
    if user['in_game']:
        if int(message.text) == user['secret_number']:
            ...
            user['attempts'] -= 1
            await message.answer(f'Загаданное число меньше {int(message.text)}\n'
                                 f'Оставшиеся попытки - {user["attempts"]}')
        elif int(message.text) < user['secret_number']:
            user['attempts'] -= 1
            await message.answer(f'Загаданное число больше {int(message.text)}\n'
                                 f'Оставшиеся попытки - {user["attempts"]}')
